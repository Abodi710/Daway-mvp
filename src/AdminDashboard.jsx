...
export default function AdminDashboard({ user, onLogout, onTokenError }) {
  ...
  const loadData = () => {
    ...
    // The route returns every application; only undecided ones need action here.
    apiFetch('/admin/pending-pharmacies')
      .then(data => setPendingPharmacies((data || []).filter(p => p.status === 'pending')))
      .catch(logError);
  };
  ...
  const handleSaveUser = (id, updatedFields) => {
    apiFetch(`/users/${id}`, { method: 'PUT', body: JSON.stringify({
      ...
      ...(updatedFields.role === 'pharmacy_staff' ? { owner_id: updatedFields.owner_id } : {}),
    }) }).then(() => {
      setEditUsr(null);
      loadData();
    }).catch(reportError);
  ...
  const handleSubmitRejection = async (reason) => {
    ...
    try {
      await apiFetch(`/admin/reject-pharmacy/${rejectPharmacyId}`, { method: 'PUT', body: JSON.stringify({ rejection_reason: reason }) });
  ...
  const handleToggleStatus = (u) => {
    ...
    setConfirmAction({
      message: suspending
        ? `إيقاف حساب ${u.email}؟ سيُسجَّل خروجه فوراً ولن يستطيع الدخول حتى إعادة التفعيل.`
        : `إعادة تفعيل حساب ${u.email}؟`,
      onConfirm: () => {
        apiFetch(`/users/${u.id}/toggle-status`, { method: 'PUT' }).then(() => {
          setConfirmAction(null);
          loadData();
        }).catch(reportError);
      },
  ...
  return (
    <div dir="rtl" className="min-h-screen bg-slate-50">
        ...
        <section className={PANEL}>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-right">
              ...
              <tbody className="divide-y divide-slate-100 text-sm text-slate-700">
                {users.length === 0 ? (
                  <tr><td colSpan="5" className={EMPTY}>لا يوجد مستخدمون لعرضهم.</td></tr>
                ) : users.map(u => (
                  <tr key={u.id} className={ROW}>
                    ...
                    <td className={TD}>
                      <span className={`inline-flex items-center rounded-full px-2.5 py-1 text-xs font-bold ring-1 ${u.status === 'suspended' ? 'bg-rose-50 text-rose-700 ring-rose-100' : 'bg-emerald-50 text-emerald-700 ring-emerald-100'}`}>
                    ...
                    <td className={`${TD} text-center`}>
                      {u.id === user.id ? (
                        ...
                        <div className="flex justify-center gap-2">
                          ...
                          <button type="button" onClick={() => handleToggleStatus(u)} className={`${ACTION} text-amber-700 hover:bg-amber-50`}>
              ...
              <tbody className="divide-y divide-slate-100 text-sm text-slate-700">
                {filteredSales.length === 0 ? (
                  ...
                  <tr key={s.id} className={ROW}>
                    ...
                    <td className={`${TD} text-xs text-slate-500`}>{formatDateTime(s.sold_at)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {editUsr && <EditUserModal user={editUsr} pharmacies={uniquePharmacies} onSave={handleSaveUser} onClose={() => setEditUsr(null)} />}
      {showAddUser && <AddUserModal pharmacies={uniquePharmacies} onSave={handleAddUser} onClose={() => setShowAddUser(false)} />}
      {rejectPharmacyId && <RejectReasonModal rejectPharmacyId={rejectPharmacyId} onRequestClose={() => setRejectPharmacyId(null)} onSubmit={handleSubmitRejection} />}
      {confirmAction && (
