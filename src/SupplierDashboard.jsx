...
import { PURCHASE_ORDER_STATUS } from './orderStatus.js';
            ...
            {loading ? <p>جاري التحميل...</p> : medicines.length === 0 ? <p>لا توجد أدوية</p> : (
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  <thead><tr><th className="text-right py-2">الاسم</th><th className="text-right py-2">السعر</th><th className="text-right py-2">الجملة</th><th className="text-right py-2">الوصف</th></tr></thead>
                  <tbody>
                    {medicines.map(m => (
                      <tr key={m.id} className="border-b">
                        <td className="py-2">{m.name}</td>
                        <td className="py-2">{formatCurrency(m.price)}</td>
                        <td className="py-2 text-xs">{m.bulk_qty ? `${formatCurrency(m.bulk_price)} لكل ${m.bulk_qty}+` : '-'}</td>
                  ...
                  <tbody>
                    {purchaseOrders.map(order => (
                      <tr key={order.id} className="border-b">
                        ...
                        <td className="py-2"><StatusBadge status={order.status} map={PURCHASE_ORDER_STATUS} /></td>
                        <td className="py-2">
