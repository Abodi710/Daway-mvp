...
import api, { clearSession, isAuthError, readUser, storeSession } from './api';
...
/**
 * The cart survives reloads and the detour through login. It holds only public
 * catalog data (ids, names, prices), so per-browser storage is appropriate;
 * every read is guarded because storage can be unavailable.
 */
const CART_KEY = 'daway_cart';

const readCart = () => {
  try {
    const items = JSON.parse(localStorage.getItem(CART_KEY) || '[]');
    return Array.isArray(items)
      ? items.filter((item) => item?.medicine?.id != null && item.medicine.pharmacyId != null && item.quantity > 0)
      : [];
  } catch {
    return [];
  }
...
export default function App() {
  ...
  const handleAuthenticated = (user, token) => {
    ...
    const adapted = storeSession(token, user) || user;
    setCurrentUser(adapted);
    // Back to where login was requested (e.g. the cart), else the dashboard.
    const from = location.state?.from;
    const back = typeof from === 'string' && from.startsWith('/') && !from.startsWith('/login') ? from : '/dashboard';
  ...
  const updateCartQuantity = (medicineId, quantity) => {
    setCartItems((items) => items.map((item) =>
      item.medicine.id === medicineId ? { ...item, quantity: Math.max(1, quantity) } : item));
  };

  const removeFromCart = (medicineId) => {
    setCartItems((items) => items.filter((item) => item.medicine.id !== medicineId));
  };

  /** Places one order for the cart lines belonging to `pharmacyId`. */
  const checkout = async (pharmacyId) => {
    if (!currentUser) {
      notify('سجّل الدخول لإتمام الطلب — سلتك محفوظة');
      navigate('/login', { state: { from: '/cart' } });
      return;
    }
    if (currentUser.role !== 'customer') {
      notify('الطلب من الصيدليات متاح لحسابات العملاء فقط');
      return;
    }
    const lines = cartItems.filter((item) => item.medicine.pharmacyId === pharmacyId);
    try {
      await api.orders.create({
        pharmacyId,
        items: lines.map((item) => ({ medicine_id: item.medicine.id, quantity: item.quantity })),
      });
      setCartItems((items) => items.filter((item) => item.medicine.pharmacyId !== pharmacyId));
      notify('تم إنشاء الطلب. ارفع إيصال بنكك من «طلباتي» لتراجعه الصيدلية');
      if (lines.length === cartItems.length) navigate('/dashboard', { state: { tab: 'orders' } });
    } catch (error) {
      if (isAuthError(error)) {
        handleTokenError();
        return;
      }
      notify(error.message || 'تعذّر إنشاء الطلب');
    }
  };
