    ...
    <div className="max-w-7xl mx-auto px-4 py-8 space-y-8">
      <div className="bg-emerald-700 text-white rounded-3xl p-6 shadow-md flex flex-col sm:flex-row justify-between items-center gap-4">
        <div>
          ...
          <p className="text-emerald-100 text-xs mt-1">المنشأة الحالية: <span className="font-bold underline">{user.pharmacy_name}</span> | الموظف المسؤول: {user.pharmacy_name || user.email}</p>
        </div>
          ...
          <div className="p-4">
            ...
            {(() => {
              const filtered = medicines.filter(m =>
                ...
                (m.barcode || m.bar_code || '').toLowerCase().includes(searchQuery.toLowerCase())
              );
              if (filtered.length === 0) {
                return <p className="text-sm text-slate-500">لا توجد أدوية تطابق البحث.</p>;
              }
              return (
