...
const FEATURES = [
  {
    ...
    description:
      'ابحث بالاسم التجاري أو المادة الفعّالة، واعرف أقرب صيدلية يتوفّر فيها الدواء الآن مع سعره بالجنيه السوداني.',
    icon: Search,
...
const AUDIENCES = [
  {
    ...
    description:
      'ابحث عن الدواء الذي تحتاجه، واعرف توفّره وسعره في الصيدليات القريبة منك قبل أن تتحرّك من مكانك.',
    points: [
      'ابحث بالاسم التجاري أو المادة الفعّالة',
      'قارن الأسعار بين الصيدليات الموثّقة',
      'اطلب التوصيل حيث تتوفّر الخدمة',
...
export default function LandingPage({ onNavigate }) {
  const [catalog, setCatalog] = useState({ medicines: [], pharmacies: [], states: [], live: false });
  ...
  useEffect(() => {
    ...
    api.catalog
      ...
      .then((data) => {
        if (cancelled) return;
        setCatalog(data);
        setMapState((current) => current || data.states[0] || '');
  ...
  const matches = useMemo(() => {
    ...
    return catalog.medicines.filter((medicine) =>
      `${medicine.name} ${medicine.genericName} ${medicine.category}`.toLowerCase().includes(term));
  }, [query, catalog.medicines]);

  /**
   * صفوف `/api/pharmacies` لا تحمل عمود ولاية بعد، لذا تُعرض دائماً؛ والترشيح
   * يعمل فقط على الصفوف التي تحمل ولاية فعلاً (العيّنة المحلية أو خادم لاحق).
   */
  const listedPharmacies = useMemo(
    () => catalog.pharmacies.filter((item) => !item.state || !mapState || item.state === mapState),
  ...
  /** مؤشرات مبنيّة على ما وصل فعلاً من الخادم. */
  const highlights = [
    { id: 'medicines', label: 'أدوية في الكتالوج', value: `${catalog.medicines.length}`, icon: Package },
    { id: 'pharmacies', label: 'صيدليات معتمدة', value: `${catalog.pharmacies.length}`, icon: Store },
    { id: 'states', label: 'تغطية الولايات', value: `${catalog.states.length}`, icon: MapPin },
  ];
                        ...
                        >
                          <span className="flex min-w-0 flex-col">
                            ...
                            <span className="truncate text-xs text-slate-500">
                              {medicine.genericName || medicine.category || 'دواء'}
                            </span>
                          </span>
                          <span className="flex shrink-0 items-center gap-2">
                            <span className="text-xs font-bold text-slate-900">
                              {formatCurrency(medicine.price, 'ج.س')}
                            </span>
                            <span
                              className={`rounded-full px-2 py-0.5 text-[10px] font-bold ${
                                medicine.inStock ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-500'
                              }`}
                            >
                              {medicine.inStock ? 'متوفّر' : 'غير متوفّر'}
                            </span>
                          </span>
                        </button>
                      </li>
            ...
            <div className="space-y-6 lg:col-span-5">
              ...
              <div className="space-y-3">
                ...
                <div role="group" aria-labelledby="state-picker-label" className="flex flex-wrap gap-2">
                  {catalog.states.slice(0, 6).map((state) => (
                    <button
                      key={state}
                      type="button"
                          ...
                          >
                            <span className="flex min-w-0 flex-col">
                              ...
                              <span className="mt-1 flex items-center gap-1 truncate text-xs text-slate-500">
                                ...
                                {[pharmacy.city, pharmacy.state].filter(Boolean).join('، ') || pharmacy.email || 'السودان'}
                              </span>
                            </span>
                            <span className="flex shrink-0 flex-col items-end gap-1">
                              ...
                              {pharmacy.rating != null && (
                                <span className="text-xs font-semibold text-amber-500">★ {pharmacy.rating}</span>
                              )}
                            </span>
                          </button>
                        </li>
                ...
                <div className="relative z-10 rounded-2xl border border-slate-700 bg-slate-800/95 p-4 backdrop-blur-sm">
                  {activePharmacy ? (
                    <div className="flex items-start justify-between gap-4">
                      <div className="min-w-0 space-y-1">
                        ...
                        <p className="flex items-center gap-1 truncate text-xs text-slate-400">
                          ...
                          {[activePharmacy.city, activePharmacy.state].filter(Boolean).join('، ')
                            || activePharmacy.email
                            || 'السودان'}
                        </p>
                      </div>
                      <button
                    ...
                    <p className="text-center text-xs text-slate-400">
                      {loading ? 'جارٍ تحميل الصيدليات…' : 'اختر ولاية لعرض الصيدليات المعتمدة.'}
                    </p>
                  )}
                </div>
              </div>
